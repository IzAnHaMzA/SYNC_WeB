...
try {
    ...
    if ($meResponse.StatusCode -eq 200) {
        Write-Host "SUCCESS: Get current user successful" -ForegroundColor Green
    ...
    Write-Host "FAILED: Get current user failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    if ($postResponse.StatusCode -eq 201) {
        Write-Host "SUCCESS: Post creation successful" -ForegroundColor Green
...
try {
    ...
    if ($feedResponse.StatusCode -eq 200) {
        Write-Host "SUCCESS: Get feed posts successful" -ForegroundColor Green
    ...
    Write-Host "FAILED: Get feed posts failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
if ($global:testPostId) {
    try {
        ...
        if ($likeResponse.StatusCode -eq 200) {
            Write-Host "SUCCESS: Like post successful" -ForegroundColor Green
        ...
        Write-Host "FAILED: Like post failed: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "SKIPPED: No post ID available" -ForegroundColor Yellow
}
...
if ($global:testPostId) {
    try {
        ...
        if ($unlikeResponse.StatusCode -eq 200) {
            Write-Host "SUCCESS: Unlike post successful" -ForegroundColor Green
        ...
        Write-Host "FAILED: Unlike post failed: $($_.Exception.Message)" -ForegroundColor Red
    }
} else {
    Write-Host "SKIPPED: No post ID available" -ForegroundColor Yellow
}
...
try {
    ...
    if ($searchResponse.StatusCode -eq 200) {
        Write-Host "SUCCESS: User search successful" -ForegroundColor Green
    ...
    Write-Host "FAILED: User search failed: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    Write-Host "FAILED: Frontend accessibility test failed: $($_.Exception.Message)" -ForegroundColor Red
}

Write-Host "`nMANUAL TESTING INSTRUCTIONS" -ForegroundColor Cyan
Write-Host "=============================" -ForegroundColor Cyan

Write-Host "`nFRONTEND TESTING (Open http://localhost:4173 in your browser):" -ForegroundColor Yellow
...
Write-Host "`nSECURITY TESTING:" -ForegroundColor Yellow
